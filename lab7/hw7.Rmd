...
sharks_fishing_pressure <- sharks %>%
  group_by(asfis_species_name) %>% 
  summarize(total_catch_sharks = sum(catch, na.rm = T)) %>% 
  mutate(fishing_pressure = ifelse(total_catch_sharks > 75, "high", "moderate"))
```
...
sharks_fishing_pressure %>% 
  ...
  summarize(average_catch_sharks = mean(total_catch_sharks, na.rm = T))
```
