...
fisheries %>% 
  ...
  summarize(total_catch_isscaap_taxonomic_group = sum(catch, na.rm = T)) %>% 
  arrange(-total_catch_isscaap_taxonomic_group)
```
...
fisheries %>% 
  ...
  summarize(most_caught_asfis_species_name = sum(catch, na.rm = T)) %>% 
  arrange(-most_caught_asfis_species_name)
```

6. For the species that was caught the most, which country had the highest catch?
```{r}
fisheries %>%
  select(asfis_species_name, country, catch) %>% 
  filter(asfis_species_name == "Engraulis ringens") %>% 
  group_by(country) %>% 
  summarize(country_total_catch = sum(catch, na.rm =T)) %>% 
  arrange(-country_total_catch)
```
Peru
...
```{r}
sharks_fishing_pressure <- sharks %>%
...
```{r}
sharks_fishing_pressure %>% 
  group_by(fishing_pressure) %>% 
  summarize(average_catch_sharks = mean(catch, na.rm = T))
```
...
fisheries %>% 
  group_by(year) %>% 
  summarize(all_country_total_catch = sum(catch, na.rm = T)) %>%
  arrange(-all_country_total_catch)
```
The intent of this code was to check which year in the data had the highest total catch across all countries. From the output, we see that 2007 had the highest catch across every country.
