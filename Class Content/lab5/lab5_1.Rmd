...
```{r}
fish$lakeid <- as.factor(fish$lakeid) #change class ofa variable
```
...
```{r}
filter(fish, lakeid == "AL" & 350 < length)
```

Notice that the `|` operator generates a different result. Why?
It appears to return results where either condition is true.
```{r}
filter(fish, lakeid == "AL" | 350 < length)
```
...
```{r}
filter(fish, !400 < length & !11 < scalelength | !8 < radii_length_mm)
```

## Practice  
1.  From the `mammals` data, filter all members of the family Bovidae with a mass greater than 450000.
```{r}
filter(mammals, family == "Bovidae", 450000 < mass)
```
...
primates <- select(primates, genus, species, mass, gestation, newborn)
```
...
```
The species maura of genus Macaca has the smallest newborn mass of 389.5.
...
```{r}
felidae <- filter(mammals, family == "Felidae" & gestation < 3)
felidae <- select(felidae, genus, family, gestation)
felidae <- filter(felidae, !gestation == -999)
felidae
```
