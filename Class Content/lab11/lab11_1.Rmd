...
life_history %>% 
  group_by(order) %>% 
  summarize(order_mean_mass = mean(mass, na.rm = T)) %>% 
  ggplot(aes(x = order, y = order_mean_mass))+
  geom_col()+
  coord_flip()
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(order_mean_mass = mean(mass, na.rm = T)) %>% 
  ggplot(aes(x = reorder(order, order_mean_mass), y = order_mean_mass))+ #stylistic changes
  geom_col()+
  coord_flip()+
  scale_y_log10()
```
...
elephants %>% 
  ...
  labs(title = "Age vs Height", y = "height in centimeters", x = "age in years")
```
...
elephants %>% 
  ...
  geom_point()+
  geom_smooth(method = lm, se = F)+
  labs(title = "Elephant Age vs Height", y = "height in centimeters", x = "age in years")+
...
elephants %>% 
  ...
  geom_bar()+
  labs(title = "Counts of Elephants by Sex", y = "Count", x = "Sex")+
  theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))+
  theme_classic()
```
