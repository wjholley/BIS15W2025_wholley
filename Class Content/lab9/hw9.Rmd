...
Yes these data are tidy as there is no data stored as column names and there are no doubled up columns. The data is in long format, which is good for manipulating it in R, but harder to look at and understand as a human.
...
```{r}
sydneybeaches_average_ent_long <- sydneybeaches_long_sep %>% 
  group_by(site, year) %>%
  summarize(average_ent = mean(enterococci_cfu_100ml, na.rm = T))
sydneybeaches_average_ent_long
```

8. Make the output from question 7 easier to read by pivoting it to wide format.
```{r}
sydneybeaches_average_ent_wide <- sydneybeaches_average_ent_long %>% 
  pivot_wider(names_from = "year",
              values_from = "average_ent")
sydneybeaches_average_ent_wide
```
...
sydneybeaches_average_ent_long %>% 
  ...
  arrange(-average_ent)
```
Little Bay Beach with an average enterococci cfu per 100ml of 121.53.

10. Explore the data! Do one analysis of your choice that includes a minimum of three lines of code.  
```{r}
sydneybeaches_average_ent_year <- sydneybeaches_long_sep %>% 
  group_by(year) %>% 
  summarize(average_ent_year = mean(enterococci_cfu_100ml, na.rm = T))
sydneybeaches_average_ent_year %>% 
  arrange(-average_ent_year)
```
