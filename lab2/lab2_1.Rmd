...
```{r}
practice <- as.numeric(practice) #this changes practice from an integer into a numeric
```
...
#this can be used to change the class of an object
```

```{r}
is.integer(my_integer) #is my_integer an integer?
```
...
I interpret this result to mean that the missing NA value causes the calculated mean to be invalid. R will not make calculations with missing pieces of data. 

4. Recalculate the mean using the following code chunk. Why is this useful?

```{r}
mean(new_vector, na.rm=T) # na.rm can be used to remove NAs from our data.
```

This is useful because it lets us calculate the mean by ignoring the NAs. It allows R to ignore the NAs.
