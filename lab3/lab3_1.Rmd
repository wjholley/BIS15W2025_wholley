...
hsprings <- tibble(name, temp, spring)
hsprings
```
...
```{r}
hsprings <- tibble(scientist=name, temp_C=temp, spring)
hsprings
```
...
hsprings <- tibble(scientist=name,temp_c=temp, spring, depth_ft)
hsprings
```
...
```{r}
mountains <- read_csv("../lab2/mountains_data.csv")
str(mountains)
```
